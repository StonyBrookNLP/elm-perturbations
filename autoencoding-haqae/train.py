...
def monolithic_compute_loss(iteration, model, target, target_lens, latent_values, latent_root, diff, dec_outputs, use_cuda, args, train=True): 
    ...
    if not args.nohier:
        ...
        commit2 = torch.zeros(1) if not use_cuda else torch.zeros(1).cuda()
...
def classic_train(args):
    ...
    vocab2 = du.create_vocab('voc.txt')
    vocab.extend(vocab2)
    with open('new_vocab', 'wb') as fi:
    ...
    dataset = du.SentenceDataset(args.train_data, vocab, args.src_seq_length, add_eos=False,train=True) 
    ...
    logger.info("Number of samples = %d"%data_len)
    ...
    for iteration, bl in enumerate(batches): #this will continue on forever (shuffling every epoch) till epochs finished
        ...
        model.zero_grad()
        latent_values, latent_root, diff, dec_outputs = model(batch, batch_lens,target,target_lens)
        ...
        #logger.info("Iteration:{}**Train loss {:.2f}.**\n".format(iteration, loss))
        ...
        #if ((args.batch_size * iteration) > data_len  or iteration % args.validate_after == 0) and iteration != 0:
        if (iteration % (data_len//args.batch_size) == 0  or iteration % args.validate_after == 0) and iteration != 0:
            print("\nFinished Training Epoch/iteration {}/{}".format(curr_epoch, iteration))

            # do validation
            print("Loading Validation Dataset.")
            val_dataset = du.SentenceDataset(args.valid_data, vocab, args.src_seq_length, add_eos=False,train=False) 
            ...
            logger.info("Iteration:{}**Validation loss {:.4f}.**\n".format(iteration, valid_loss[0]))

            # Check max epochs and break
            #if (args.batch_size * iteration) % data_len == 0:
            #if (args.batch_size * iteration) > data_len:
            if iteration % (data_len//args.batch_size) == 0:
                torch.save(model, os.path.join(args.save_model,"{}_{}_{}_.epoch_{}.loss_{:.2f}.pt".format(args.save_model, args.commit_c, args.commit2_c,curr_epoch, float(valid_loss[0]))))
                torch.save(optimizer, os.path.join(args.save_model,"{}.{}.epoch_{}.loss_{:.2f}.pt".format(args.save_model, "optimizer", curr_epoch, float(valid_loss[0]))))
                curr_epoch += 1
    ...
    torch.save(model, os.path.join(args.save_model,"{}_{}_{}_.epoch_{}.loss_{:.2f}.pt".format(args.save_model, args.commit_c, args.commit2_c,curr_epoch-1, float(valid_loss[0]))))
    torch.save(optimizer, os.path.join(args.save_model,"{}.{}.epoch_{}.loss_{:.2f}.pt".format(args.save_model, "optimizer", curr_epoch-1, float(valid_loss[0]))))
