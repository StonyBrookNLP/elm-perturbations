...
#Type of perturbation applied to the data
permuted = False
masked = False
dropped = False
...
class SentenceDataset(ttdata.Dataset):
    ...
    def __init__(self, path, vocab, src_seq_length=50, min_seq_length=8, n_cloze=False, add_eos=True, train=True):
        ...
        with open(path, 'r') as f:
            for line in f:
                ...
                if n_cloze:
                    ...
                    if not (permuted or masked or dropped):
                        examples.append(ttdata.Example.fromlist([text, text], fields))
                    
                    elif permuted and train:
                        if counter%percent != 0:
                            examples.append(ttdata.Example.fromlist([text, text], fields))
                        
                        elif counter%percent == 0:
                            sample_split = text.split("<TUP>") 
                            ...
                            if len(sample_split) >= 2:
                                ...
                                input_seq_reversed_text = ' '.join(input_seq_reversed_text.split())
                                examples.append(ttdata.Example.fromlist([input_seq_reversed_text, text], fields))
                            ...
                            if len(sample_split) >= 4:
                                ...
                                text_new = ' <TUP> '.join(odd_elements+even_elements)
                                #text = text + " <TUP>"
                                text_new = ' '.join(text_new.split())
                                examples.append(ttdata.Example.fromlist([text_new, text], fields))
                                ...
                                text_new = ' <TUP> '.join(odd_elements_reversed+even_elements_reversed)
                                #text = text + " <TUP>"
                                text_new = ' '.join(text_new.split())
                                examples.append(ttdata.Example.fromlist([text_new, text], fields))
                                   
                    
                    elif (masked or dropped) and train:
                        if counter%percent != 0:
                            examples.append(ttdata.Example.fromlist([text, text], fields))
                            sample_split = text.split("<TUP>") 
                            ...
                            repetition = True
                            if len(sample_split) > 2:
                                for k in range(len(sample_split)//4):
                                    repetition = True
                                    ...
                                    if not repetition:
                                        ...
                                        input_text = ' '.join(input_text.split())
                                        examples.append(ttdata.Example.fromlist([input_text, text], fields))

                        
                        elif counter%percent == 0:
                            ## adding the original sequence
                            examples.append(ttdata.Example.fromlist([text, text], fields))
                            ...
                            if len(sample_split) > 2:
                                for k in range(len(sample_split)//4):
                                    ...
                                    if not repetition:
                                        ...
                                        input_seq_reversed = sample_split_new[::-1]
                                        input_seq_reversed_text = ' <TUP> ' .join(input_seq_reversed)
                                        #input_seq_reversed_text = input_seq_reversed_text + " <TUP> "
                                        input_seq_reversed_text = ' '.join(input_seq_reversed_text.split())

                                        output_seq_reversed = sample_split_new[::-1]
                                        output_seq_reversed_text = ' <TUP> '.join(output_seq_reversed)
                                        #output_seq_reversed_text = output_seq_reversed_text + " <TUP> "
                                        output_seq_reversed_text = ' '.join(output_seq_reversed_text.split())
                                        examples.append(ttdata.Example.fromlist([input_seq_reversed_text, text], fields))

                                        ## adding two more permutations
                                        if len(sample_split)>=4:
                                            # add the sequence of even elements accompanied by odd elements
                                            even_elements = sample_split_new[::2]
                                            odd_elements = sample_split_new[1::2]
                                            text_new = ' <TUP> '.join(odd_elements+even_elements)
                                            text_new = ' '.join(text_new.split())
                                            examples.append(ttdata.Example.fromlist([text_new, text], fields))
        ...
        with open(path, 'r') as f: 
            for idx, line in enumerate(f):
                ...
                if easy: # EASY narrative cloze task. WONT ADD ROLE FOR THIS AS NOT USED ANYMORE.
                    ...
                    ##actual = sents[0].strip().split(TUP_TOK)[-2:]
                    ##actual = TUP_TOK.join(actual)
                    
                    dists = sents[1:]
                    
                    #seed = actual.split(TUP_TOK) 
                    
                    seed = sents[0].split(TUP_TOK) 
                    ...
                    # actual_list = actual.split(TUP_TOK)[1:]
                    # actual_list = actual_list[::-1]
                    # even_elements = actual_list[::2]
                    # odd_elements = actual_list[1::2]
                    # actual_list = odd_elements+even_elements
                    # np.random.shuffle(actual_list)
                    # actual = ' <TUP> '.join(actual_list)
                    # actual = ' '.join(actual.split())
                    # actual = "{} {} {}".format(seed, TUP_TOK, actual)
                    data_list = [actual, actual]
                    for dist in dists:
                        # dist_list = dist.split(TUP_TOK)
                        # dist_list = dist_list[::-1]
                        # even_elements = dist_list[::2]
                        # odd_elements = dist_list[1::2]
                        # dist_list = odd_elements+even_elements
                        # np.random.shuffle(dist_list)
                        # dist = ' <TUP> '.join(dist_list)
